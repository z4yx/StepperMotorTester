    ...
    for (int i = 0; i < NUM_KEYS; ++i)
    {
        ...
        if(cur_state != bPressed[i]) {
            ...
            if(now - lastChange[i] >= KEYBOARD_MIN_TOGGLE_PERIOD){
                ...
                LOG_DBG("State changed key=%d, state=%d", (int)i, (int)cur_state);
                KeyBoard_EventHandler(i, cur_state==1?KEYEVENT_DOWN:KEYEVENT_UP);
            }
        }else{
            //长时间按下,开始自动重复
            if(cur_state && now - lastChange[i] >= KEYBOARD_START_REPEAT_TIME) {
                if(now - lastRepeat[i] >= KEYBOARD_REPEAT_PERIOD){
                    lastRepeat[i] = now;
                    LOG_DBG("Repeat key=%d, state=%d", (int)i, (int)cur_state);
                    KeyBoard_EventHandler(i, KEYEVENT_REPEAT);
                }
            }
        }
    }
